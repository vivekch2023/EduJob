...
export default function FreePdfManager() {
  const { id } = useParams();
  ...
  const fetchPdfs = async () => {
    const res = await axios.get(`http://localhost:5000/api/courses/${id}/free`);
    setPdfs(res.data);
  };
  ...
  const addPdf = async (e) => {
    ...
    const updatedCourse = { ...courseRes.data, free: [...courseRes.data.free, { title, url }] };
  ...
  const deletePdf = async (index) => {
    ...
    updatedCourse.free.splice(index, 1);
    await axios.put(`http://localhost:5000/api/courses/${id}`, updatedCourse);
    fetchPdfs();
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">Manage Free PDFs</h1>
      </div>
